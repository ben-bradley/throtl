...
function Valve(options) {
  ...
  if (isNaN(limit) || limit <= 0)
    throw new Error('valve limit must be a number greater than 0');

  var pending = 0,
    valve = new Stream.PassThrough(options);

  valve.paused = false;
  valve._paused = 0;
  valve._resumed = 0;

  valve.tick = function(data) {
    if (--pending < limit && valve.paused) {
      valve.paused = false;
      valve._resumed += 1;
      valve.resume();
    }
  ...
  valve.next = new Stream.PassThrough(options);

  valve.next.on('data', valve.tick);
